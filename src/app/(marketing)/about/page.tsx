    ...
    <main>
      ...
      <section style={{ padding: '72px 24px', background: '#FFFFFF' }}>
        <div style={{ maxWidth: 760, margin: '0 auto' }}>
          ...
          <h2 style={{ fontSize: 'clamp(22px, 3vw, 28px)', fontWeight: 800, color: C.text, margin: '0 0 20px', letterSpacing: '-0.01em' }}>
            One platform, built for every agency
          </h2>
          <div style={{ fontSize: 15, color: C.textSub, lineHeight: 1.8 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(230px, 1fr))', gap: 20 }}>
            {PRINCIPLES.map(p => (
              <div key={p.title} style={{ background: '#FFFFFF', padding: 22, borderRadius: 10, border: `1px solid ${C.border}` }}>
                <h3 style={{ fontSize: 15, fontWeight: 700, color: C.text, margin: '0 0 8px' }}>{p.title}</h3>
                <p style={{ fontSize: 13, color: C.textSub, lineHeight: 1.6, margin: 0 }}>{p.body}</p>
              </div>
            ))}
          </div>
        </div>
